...
3. **rag_system.py** orchestrates: gets conversation history, calls `ai_generator.generate_response()` with the `search_course_content` tool definition
...
5. **search_tools.py** `CourseSearchTool.execute()` calls `vector_store.search()`, which does semantic similarity search on ChromaDB, then formats results and tracks sources
...
- **Tool-based search**: Claude decides when to search via Anthropic tool calling (`search_course_content`), rather than always searching. General knowledge questions get direct answers.
